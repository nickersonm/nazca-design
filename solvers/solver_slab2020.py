...
class SlabMode(SolverBase):
    ...
    def calculate_mode(self, wl=None, pol=None, mode=None, **kwargs):
        ...
        if hasattr(self.Nr, "Neff"):
            Nr = self.Nr.Neff(wl=self.wl, pol=self.pol, mode=0, **kwargs)
        elif callable(self.Nr):
            Nr = self.Nr(wl=self.wl, pol=self.pol, mode=0, **kwargs)
        else:
            Nr = self.Nr

        if hasattr(self.Nbg, "Neff"):
            Nbg = self.Nbg.Neff(wl=self.wl, pol=self.pol, mode=0, **kwargs)
        elif callable(self.Nbg):
            Nbg = self.Nbg(wl=self.wl, pol=self.pol, mode=0,  **kwargs)
        else:
            Nbg = self.Nbg
        assert Nbg > 0

        if hasattr(self.Nsub, "Neff"):
            Nsub = self.Nsub.Neff(wl=self.wl, pol=self.pol,  **kwargs)
        elif callable(self.Nsub):
            Nsub = self.Nsub(wl=self.wl, pol=self.pol,  **kwargs)
        else:
            Nsub = self.Nsub
    ...
    def Neff(self, wl=None, pol=None, mode=None, **kwargs):
        ...
        if self.doRecalc:
            ...
            if not isinstance(self.mode, list):
                ...
                modes = self.mode
            for m in modes:    
                self.calculate_mode(wl=self.wl, pol=self.pol, mode=m, **kwargs)
