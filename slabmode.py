...
class EIM(object):
    ...
    def __init__(self, Mr=None, Mb=None, w=2.0, wl=sim.wl, name=None, points=100):
        self.view = "topview"
        ...
        self.EFieldResolution = 0.05 # in [um] resolution  when plotting a field profile
    ...
    def __getPe(self):
        ...
        Per = self.__Ge*self.__Ge*(self._w/2 + m.sin(self.__c*self._w)/(2*self.__c)) # ridge
        self.__Pe = Per + Pet
        return self.__Pe
    ...
    def __getPo(self):
        ...
        if self._clip/2 > self._w/2:
            Pot = Pot - Ho*m.exp(self.__u*(self._w-self._clip))/self.__u #tail correction when clipping domain at x0
        Por = self.__Go*self.__Go*(self._w/2 - m.sin(self.__c*self._w)/(2*self.__c)) #part under ridge
        self.Po = Por + Pot
        return self.Po
    ...
    def __calculateMode(self, **kwargs):
        ...
        self._V = (2*m.pi*self._w/self._wl)*m.sqrt(self._Nridge**2 - self._Nbg**2)
        self._maxmodes = int(self._V/m.pi)+1; # one polarization
        ...
        dbmin = 1e-12 #needs detail in case of very wide slabs, e.g. Wslab = 1000*lambda
        db = (1.0-b)-dbmin
        value = 0
        ...
        if self.__polRot(self._pol) == 0: # TE, polRot for topview
            ...
            print ('bpMessage.Error ("(slabmode): provided unknown polarization to function InitMode().")')
        if count >= countMax:
            print ('(slabmode): Too many iterations: Maybe the ')

        self._b = b
        self._Neff = m.sqrt(self._b*(self._Nridge**2-self._Nbg**2) + self._Nbg**2);
    ...
    @property
    def b(self):
        if self.__doRecalc: self.__calculateMode();
        return self._b
    ...
    def initSlab(self,
                 ...
                 pol=0,  # polarization */
                 mode=0, # mode number
                 clip=0.0): # where to force the modeprofile to 0 (clipping) */
    ...
    def __getEe(self, xx):
        ...
        for x in xx:
            if x < -self._w/2.0:
                X.append(self.__Je*m.exp(self.__u*(x+self._w/2.0))/self.__sqrtPe)
            elif x > self._w/2.0:
                X.append(self.__Je*m.exp(-self.__u*(x-self._w/2.0))/self.__sqrtPe)
            else:
    ...
    def __getEo(self, xx):
        ...
        for x in xx:
            if x < -self._w/2.0:
                X.append(-self.__Jo*m.exp(self.__u*(x+self._w/2.0))/self.__sqrtPo)
            elif x > self._w/2.0:
                X.append(self.__Jo*m.exp(-self.__u*(x-self._w/2.0))/self.__sqrtPo)
            else:
    ...
    def printModeInfo(self, mode=-1):
        ...
        "\n- wl            = ", self._wl,\
        "\n- pol           = ", self._pol,\
        "\n- guided-modes  = ", self.numberOfGuidedModes,\
        "\n- Nr            = ", self._Nridge,\
        "\n- Nbg           = ", self._Nbg,\
        "\n- Nsub          = ", self._Nsub,\
        "\n- beta          = ", self.beta,\
    ...
    def __str__(self):
        ...
        out += '\n- materials                  - M       : ' + ', '.join([self.Mb.name, self.Mr.name, self.Mb.name])
        out += '\n- layer thickeness           - D [um]  : ' + str(self.w)
        out += '\n- refractive indices         - N       : ' + ', '.join(map(str,[self.Mb.Neff(), self.Mr.Neff(), self.Mb.Neff()]))
