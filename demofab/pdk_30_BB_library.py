...
# ==============================================================================
shallow = nd.interconnects.Interconnect(
    xs="Shallow",
    varname="shallow",
    doc="Shallow waveguide",
)  #: 'Shallow' interconnect
deep = nd.interconnects.Interconnect(
    xs="Deep",
    varname="deep",
    doc="Deep waveguide",
)  #: 'Deep' interconnect
metaldc = nd.interconnects.Interconnect(
    xs="MetalDC",
    varname="metaldc",
    doc="Metal DC line",
)  #: 'MetalDC' interconnect
metalrf = nd.interconnects.Interconnect(
    xs="MetalRF",
    varname="metalrf",
    doc="Metal RF line",
...
def abb_eopm_dc(length=750, contacts=2):
    ...
    with nd.Cell(hashme=True) as C:
        ...
        p2 = nd.Pin(name="b0", xs="Deep", width=wdeep, remark="optical").put(
            length, 0, 0
        )
        ...
        icons.icon_strt(length, width, bufx=0, bufy=10, layer="MetalDCIcon").put(
            "cc", 0, "cc"
        )
        icons.icon_strt(length, width, bufx=10, bufy=-8, layer="DeepIcon").put(
            "cc", 0, "cc"
        )
    return C
...
def soa(length=100, pad=True):
    ...
    with nd.Cell(hashme=True) as C:
        ...
        icons.icon_strt(length, width, bufx=10, bufy=-8, layer="ActiveIcon").put(
            "cc", 0, h, "cc"
        )
    return C
...
def phase_shifter(length=100):
    ...
    nd.rangecheck(
        {
            "length": nd.varinfo(
                10, length, 2000, "float", "um", "length of phase_shifter"
            ),
        }
    )
...
def Tp_isolation(name="isolation", length=100, xs=None, width=None, layer=None):
    ...
    def isolation(length=length):
        ...
        nd.rangecheck(
            {
                "length": nd.varinfo(
                    0, length, 2000, "float", "um", "length of isolation"
                ),
            }
        )
        ...
        with nd.Cell(hashme=True) as C:
            ...
            icons.icon_strt(length, bb_width, bufx=4, bufy=-8, layer=layer).put(
                "cc", 0, "cc"
            )
        return C
...
isolation_sh = Tp_isolation(
    name="isolation_sh", xs="Shallow", width=wshal, layer="ShallowIcon"
)
BBfunctioncalls.append("isolation_sh()")
isolation_act = Tp_isolation(
    name="isolation_act", xs="Active", width=wact, layer="ActiveIcon"
)
...
def eopm_dc(length=1000, pads=False, sep=10):
    ...
    varinfo = {
        ...
        "sep": nd.varinfo(
            ...
            type="float",
            unit="um",
            doc="separtions of pads from the soa",
        ),
    }
...
def io(shape=None, width=3.0, bend=False, deep=False):
    ...
    if shape not in options:
        print(
            "Error: io shape '{}'not recognized. Options are: {}".format(shape, options)
        )
...
def dbr_laser(Ldbr1=50, Ldbr2=500, Lsoa=750, Lpm=70):
    ...
    with nd.Cell(hashme=True) as laser:
        ...
        pdk.put_boundingbox(
            "org", length=abs(length), width=200, align="lb", move=(0, -30, 0)
        )
    return laser
...
def mzi(length=1000, sep=50):
    """Creates a simmetric MZI with phase shifters in both arms
...
def awg1x4():
    ...
    with nd.Cell(hashme=True) as awg1x4BB:
        awg1x4BB.version = version
        awg = nd.load_gds(
        ...
        nd.connect_path(awg1x4BB.pin["a0"], awg1x4BB.pin["b0"], 300, [(500, 140)])
        nd.connect_path(awg1x4BB.pin["a0"], awg1x4BB.pin["b1"], 500, [(600, 140)])
        nd.connect_path(awg1x4BB.pin["a0"], awg1x4BB.pin["b2"], 700, [(700, 140)])
        nd.connect_path(awg1x4BB.pin["a0"], awg1x4BB.pin["b3"], 900, [(800, 140)])
